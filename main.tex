...
\lstdefinestyle{latex}{
    ...
    emphstyle=\TrueBlue,
    %%%%%
    emph={LaTeX, usepackage, textit, textbf, textsc, title, date, author}
}
...
\begin{document}
    ...
    Sleek Template is a minimal collection of \LaTeX{} packages and settings that ease the writing of beautiful documents. While originally meant for theses, it is perfectly suitable for project reports, articles, syntheses, etc. -- with a few adjustments, though.
    ...
    \begin{note}
        If you are a \LaTeX{} beginner consider the excellent \href{https://www.overleaf.com/learn}{Overleaf tutorial}. Also, there are a lot of symbols available in \LaTeX{} and, therefore, in this template. I recommend the use of \enquote{The Comprehensive \LaTeX{} Symbol List} \cite{pakin2020comprehensive} for searching symbols.
    \end{note}
    ...
    A few custom macros have also been added such as \texttt{\textbackslash{}rbk}, \texttt{\textbackslash{}sbk} and \texttt{\textbackslash{}cbk} for respectively round, square and curly brackets, \texttt{\textbackslash{}abs} for absulute value, \texttt{\textbackslash{}norm} for norm, and \texttt{\textbackslash{}fact} for factorial and \texttt{\textbackslash{}diff} for up-right differential.
    $$
    ...
    \begin{align*}
        \frac{\diff}{\diff x} \ln x & = \frac{1}{\abs{x}} \\
    ...
    The \texttt{siunitx} package provides three commands to typeset numbers and quantities -- \texttt{\textbackslash{}num}, \texttt{\textbackslash{}si} and \texttt{\textbackslash{}SI} -- as well as various units (\emph{cf.} Table \ref{tab:siunitx_units}).
    
    It is possible to write numbers without units (\emph{e.g.} \num{1}, \num{1.0}, \num{-1}, \num{3.14159}, \num{e100}, \num{6.022e23}), units without quantity (\emph{e.g.} \si{\joule}, \si{\newton\meter}, \si{\kilogram\meter\squared\per\second\squared}) and, finally, quantities with their units (\emph{e.g.} \SI{9.81}{\meter\per\second\squared}, \SI{299.6e6}{\meter\per\second}).
    ...
    In a list, each element is preceded by the command \texttt{\textbackslash{}item}. It is possible to modify the labels
    \begin{itemize}
        \item individually with \texttt{\textbackslash{}item[newLabel]};
        \item for the whole environment with the \texttt{label=newLabel} option.
    \end{itemize}
    
    In the case of \texttt{enumerate}, \texttt{newLabel} can contain special expressions (\emph{cf.} Table \ref{tab:enumerate_special_expressions}) that will adapt to the item number. For example, \texttt{label=(\textbackslash{}alph*)} defines the label sequence \enquote{(a), (b), (c), ...}. Still in the case of \texttt{enumerate}, the  \texttt{\textbackslash{}setcounter} and \texttt{\textbackslash{}addtocounter} commands allow to modify the current item number.
    
    One could also want to reduce the space between items with the \texttt{noitemsep} option or to delete the left margin with the \texttt{leftmargin=*} option.
    ...
    Thanks to the \texttt{graphicx} package, it is possible to include external graphic documents (images, plots, etc.) in your document with the \texttt{\textbackslash{}includegraphics} command. Most image type format (\texttt{jpg}, \texttt{png}, \texttt{bmp}, etc.) are supported by this command. However, it should be noted that it is highly preferable to use vectorial types, such as \texttt{pdf} or \texttt{eps}.
    ...
    Sleek Template offers a custom title-page in the package \texttt{sleek-title}. The formatting of the title-page is automatically inferred from the fields that the user has provided.
    
    The fields are \texttt{\textbackslash{}logo}, \texttt{\textbackslash{}institute}, \texttt{\textbackslash{}faculty}, \texttt{\textbackslash{}department}, \texttt{\textbackslash{}title}, \texttt{\textbackslash{}subtitle}, \texttt{\textbackslash{}author}, \texttt{\textbackslash{}supervisor}, \texttt{\textbackslash{}context} and \texttt{\textbackslash{}date}.
    
    Among these, only \texttt{\textbackslash{}title}, \texttt{\textbackslash{}author} and \texttt{\textbackslash{}date} have to be provided. However, none of the fields should stay empty. Prefer deleting or commenting the line if so.
    ...
    \begin{itemize}
        \item \texttt{\textbackslash{}NumberStyle\{stylename\}} creates a style \texttt{stylenameNumber} with line numbering;
        \item \texttt{\textbackslash{}FrameStyle\{stylename\}} creates a style \texttt{stylenameFrame} with an all around frame;
        \item \texttt{\textbackslash{}FrameTBStyle\{stylename\}} creates a style \texttt{stylenameFrameTB} with top and bottom line rules;
        \item \texttt{\textbackslash{}FrameNumberStyle\{stylename\}} and \texttt{\textbackslash{}FrameTBNumberStyle\{stylename\}} have the same logic.
    \end{itemize}
    
    For example, the \texttt{\textbackslash{}FrameTBStyle\{python\}} command creates the \texttt{pythonFrameTB} style, which can then be used to showcase \texttt{Python} code.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{ll}
            ...
            \midrule
            \texttt{\textbackslash{}arabic*} & Arabic numbers (1, 2, 3, ...) \\
            \texttt{\textbackslash{}alph*} & Lowercase letters (a, b, c, ...) \\
            \texttt{\textbackslash{}Alph*} & Uppercase letters (A, B, C, ...) \\
            \texttt{\textbackslash{}roman*} & Lowercase Roman numerals (i, ii, iii, ...) \\
            \texttt{\textbackslash{}Roman*} & Lowercase Roman numerals (I, II, III, ...) \\
            \bottomrule
        \end{tabular}
